<template>
        <div v-if="workout" class="w-screen flex justify-center items-center bg-color rounded-b-lg mb-32 md:mb-40">
            ...
            <div class="container mx-auto  rounded-b-lg overflow-hidden shadow-lg my-2 bg-white bg-color">
                <div class="relative pb-6 w-full h-80 main-bg-color workout-image-bg"
                    :style="{ 'background': 'linear-gradient(rgb(0 0 0 / 0%), rgb(0 0 29)), url(' + require('@/assets/fitness-img/workout-images/' + workout.image) + ')'}"
                    >
                    <!-- <img class="w-full" src="/public/resources/fitness/cardioIMG-1.jpg"
                  alt="Profile picture" /> -->
                    <div class="text-center absolute w-full " style="bottom: -30px">
                        <div class="mb-4">
                            <p class="text-white tracking-wide uppercase text-lg font-bold md:text-3xl">{{ workout.title }}</p>
                            <p class="text-gray-400 md:py-2 text-md md:text-xl">{{ workout.duration }},  {{ workout.title }}</p>
                        </div>
                        <button
                ...
                <div class=" main-bg-color">
                    ...
                    <div class="w-full md:w-4/5 m-auto  px-5">
                        ...
                        <span class="flex flex-row w-full h-20 ">
                         ...
                         <img class="w-8 md:w-14" :src="require(`@/assets/fitness-img/workout-images/` + workout.focusIcon) " alt="">   

                        <h1 class="text-center text-md md:text-2xl ml-5 flex items-center text-white">{{workout.bodyFocus}}</h1>
                    </span>
                        
                        <span class="flex flex-row">
                            <h1 class="gray-text-color text-md md:text-2xl py-2"> Calories Burn: </h1>
                            <h1 class="text-white text-md md:text-2xl py-2 ml-2"> {{ workout.caloriesBurn }} </h1>
                        </span>
                        <span class="flex flex-row">
                            <h1 class="gray-text-color text-md md:text-2xl py-2"> Difficulty: </h1>
                            <h1 class="text-white text-md md:text-2xl py-2 ml-2"> {{ workout.difficulty }} </h1>
                        </span>
                        <span class="flex flex-row">
                            <h1 class="gray-text-color text-md md:text-2xl py-2"> Equipment: </h1>
                            <h1 class="text-white text-md md:text-2xl py-2 ml-2"> {{ workout.equipment }} </h1>
                        </span>
                        <span class="flex flex-row">
                            <h1 class="gray-text-color text-md md:text-2xl py-2"> Training Type: </h1>
                            <h1 class="text-white text-md md:text-2xl py-2 ml-2"> {{ workout.trainigType}} </h1>
                        </span>                       
                    </div>
                    ...
                    <p class="text-lg md:text-xl px-5 py-5 text-justify md:w-4/5 m-auto text-white no-word-spacing"> {{ workout.description }} </p>
